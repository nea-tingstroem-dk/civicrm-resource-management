  ...
  controller: 'Resource_managementResourceEvent',
    ...
    var ts = $scope.ts = CRM.ts('resource-management');
      ...
      if (tablinks[j].name == "tab_" + tab) {
      tablinks[j].style.backgroundColor = "#F0F8FF";
      } else {
      ...
      }, 'id').then(function (events) {
      $scope.existingRepeats = events;
      }, function (failure) {
      // handle failure
      });
      };
      $scope.changeSelectedEvent = function (event_id) {
      $scope.selectedEventId = event_id;
        $scope.selectedEvent = $scope.existingRepeats[event_id];
      };
      ...
      //
      $scope.masterEventSelected = function () {
      event.preventDefault();
        $scope.selectedEvent = null;
        $scope.selectedEventParticipants = [];
        $scope.eventSelected($scope.masterEventId);
      };
      //
      //
      //
      $scope.eventSelected = function (eventId) {
      ...
      var event = events[0];
        ...
        if (event['Listefelter.Feltgrupper'] &&
          event['Listefelter.Feltgrupper'] !== $scope.fieldGroupId) {
      $scope.getCustomFields();
      } else {
      $scope.ParticipantsChanged();
      }
      ...
      select: ["title",
        ...
        "parent_event_id",
        "Listefelter.Feltgrupper"],
        where: [["parent_event_id", "=", event['parent_event_id']]],
        orderBy: {"start_date": "ASC"}
      }, "id").then(function (repeats) {
      $scope.existingRepeats = repeats;
      }, function (failure) {
      // handle failure
      });
      } else {
      $scope.existingRepeats[event['id']] = event;
      }
      }, function (failure) {
      // handle failure
      });
      ...
      select: ["name", "label", "custom_group_id:name", "column_name"],
        where: [
        ["custom_group_id", "=", $scope.selectedEvent['Listefelter.Feltgrupper']]
        ],
      ...
      name: field['name'],
        label: field['label',
          column: field['custom_group_id:name'] + '.' + field['column_name']]
      };
      }
    ...
    //
    //
    $scope.ParticipantsChanged = function () {
    $select = ["contact_id.external_identifier", "contact_id.display_name", "role_id"];
      if ($scope.fieldGroupId) {
    $scope.fieldMap.forEach((f) => select[] = f['column']);
    }
    crmApi4('Event', 'get', {
    select: ["title", "start_date", "end_date",
      "p_res.id", "p_resp.id",
...
//
    ...
    crmApi4('ResourceCalendarSettings', 'get', {
    select: ["config_value"],
      where: [["calendar_id", "=", $scope.calendar_id]]
    }, "config_key").then(function (rcs) {
    $scope.calendarSettings = rcs;
    }, function (failure) {
// handle failure
    });
    });
