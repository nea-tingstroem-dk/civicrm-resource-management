...
class CRM_ResourceManagement_ExtensionUtil {
  ...
  const QUEUE_NAME = 'resource-management';
...
spl_autoload_register('_resource_management_civix_class_loader', TRUE, TRUE);

function _resource_management_civix_class_loader($class) {
  if ($class === 'CRM_ResourceManagement_DAO_Base') {
    if (version_compare(CRM_Utils_System::version(), '5.74.beta', '>=')) {
      class_alias('CRM_Core_DAO_Base', 'CRM_ResourceManagement_DAO_Base');
      // ^^ Materialize concrete names -- encourage IDE's to pick up on this association.
    }
    else {
      $realClass = 'CiviMix\\Schema\\ResourceManagement\\DAO';
  ...
  if (strpos($class, 'CiviMix\\Schema\\ResourceManagement\\') === 0) {
